...
vi.mock('@/features/auth/hooks/useAuth', () => ({
  useAuth: () => ({ user: { role: UserRole.ORG_ADMIN } }),
}));
...
describe('sidebar employer — mục nav Thông báo vs chuông xem nhanh', () => {
  ...
  it.each(['vi', 'en'] as const)('[%s] hai dòng mang nhãn KHÁC nhau', (language) => {
    ...
    expect(navLink).toHaveAttribute('href', '/employer/notifications');
  ...
  it('khi thu gọn, tooltip của hai dòng vẫn khác nhau và chuông vẫn còn', () => {
    ...
    expect(screen.getByTitle('Thông báo')).toHaveAttribute('href', '/employer/notifications');
  ...
  it('mục nav dùng icon Inbox, khác icon Bell của chuông xem nhanh', () => {
    ...
    const notificationsItem = items.find((item) => item.to === '/employer/notifications');
