          ...
          <section className="space-y-4 rounded-xl border border-subtle bg-surface-raised p-6">
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between gap-3">
                ...
                <span className="text-lg font-semibold text-foreground">${order.amountUsd.toFixed(2)}</span>
              </div>
            </div>
            <p className="text-xs text-muted-foreground">{t('payment.checkout.payosHint')}</p>
            <button type="button" className="btn-primary w-full" disabled={isRedirecting} onClick={handlePay}>
