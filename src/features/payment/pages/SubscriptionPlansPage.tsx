  ...
  return (
    <div className="h-full overflow-y-auto bg-surface-base">
      <div className="page-container page-section mx-auto max-w-7xl space-y-8">
        ...
        {oneTime.length ? <PackageGroup title={t('payment.plans.oneTime')} icon={Package} packages={oneTime} showContinue onBuy={(id) => navigate(`/candidate/payment?packageId=${encodeURIComponent(id)}`)} /> : null}
...
function PackageGroup({ title, icon: Icon, packages, showContinue = false, onBuy }: { title: string; icon: typeof Package; packages: PackageResponse[]; showContinue?: boolean; onBuy: (id: string) => void }) {
  const { t } = useLanguage();
  return (
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {packages.map((item, index) => (
          <article key={item.id} className="frame-satin flex flex-col rounded-2xl bg-surface-raised p-6">
            ...
            <button type="button" className="btn-primary mt-6 w-full" onClick={() => onBuy(item.id)}>{showContinue && index === 0 ? t('payment.plans.continueCheckout') : t('payment.plans.buyPackage')}</button>
          </article>
        ))}
      </div>
    </section>
