...
export const ForgotPasswordForm: React.FC<ForgotPasswordFormProps> = ({ isSignUp, isForgotPassword, onBackToSignInClick }) => {
  ...
  return (
    <div className={`absolute inset-0 flex flex-col items-center justify-center px-12 transition-all duration-700 delay-100 ${(!isSignUp && isForgotPassword) ? 'opacity-100 translate-x-0' : 'opacity-0 pointer-events-none translate-x-[10%]'}`}>
      ...
      <input 
        className="bg-slate-100 border-none px-5 py-3.5 rounded-xl w-full mb-6 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-pine/30 transition-all placeholder:text-slate-400" 
        placeholder={t('auth.emailPlaceholder')}
      />
      
      <button className="bg-pine text-white px-12 py-3.5 rounded-xl font-bold uppercase tracking-wider hover:bg-pine/90 active:scale-95 transition-all shadow-lg shadow-pine/30 w-full mb-6">
        {t('auth.sendLink')}
      </button>

      <button 
        onClick={(e) => { e.preventDefault(); onBackToSignInClick(); }}
        className="text-sm font-medium text-slate-500 hover:text-pine transition-colors flex items-center space-x-2"
      >
