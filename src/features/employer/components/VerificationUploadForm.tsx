...
function isVerificationLocked(status: VerificationRecord['status']) {
...
export function VerificationUploadForm({ verification, onSubmit }: VerificationUploadFormProps) {
  ...
  const isReadOnly = isVerificationLocked(verification.status);
  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      {isReadOnly ? (
        <Alert>
          <AlertDescription>{t(`employer.verify.${verification.status}Copy`)}</AlertDescription>
        </Alert>
      ) : null}
      ...
      <fieldset disabled={isReadOnly} className="space-y-5 disabled:opacity-70">
        ...
        <Button type="submit" loading={form.formState.isSubmitting} className="w-full sm:w-auto" disabled={isReadOnly}>
