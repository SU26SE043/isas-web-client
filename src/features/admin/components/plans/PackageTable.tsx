...
export function PackageTable({ packages, plans, busyId, onEdit, onHide, onRestore }: PackageTableProps) {
  ...
  return (
    <div className="overflow-x-auto">
      <Table className="min-w-[880px]" aria-label={t('admin.plans.tab.packages')}>
        <TableHeader><TableRow>
          ...
          <TableHead>{t('admin.plans.package.content')}</TableHead><TableHead>{t('admin.plans.package.plan')}</TableHead><TableHead>{t('admin.plans.package.audience')}</TableHead><TableHead>{t('admin.plans.table.status')}</TableHead><TableHead className="text-right">{t('admin.plans.table.actions')}</TableHead>
