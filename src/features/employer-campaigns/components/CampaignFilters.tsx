...
export function CampaignFilters({ value, onChange }: CampaignFiltersProps) {
  ...
  return (
    <section className="frame-satin grid gap-3 rounded-xl bg-surface-raised p-3.5 lg:grid-cols-[minmax(240px,1fr)_auto] lg:items-center">
      ...
      {/* Một bộ lọc trạng thái duy nhất (chip, 1 click, aria-pressed). Trước đây còn một <select> ghi cùng
          value.status đứng cạnh ⇒ hai điều khiển cho một việc, và select có 2 trạng thái chip thiếu. */}
      <div className="flex min-w-0 flex-wrap gap-2" role="group" aria-label={t('employer.campaigns.list.status')}>
