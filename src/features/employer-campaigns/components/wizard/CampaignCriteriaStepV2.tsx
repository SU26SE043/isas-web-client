...
export function CampaignCriteriaStepV2({ rubric, campaignId, jobCategory: initialJobCategory, error, onChangeRubric, onReset, onBack, onNext, isSaving }: Props) {
  ...
  return <SectionPanel icon={<ClipboardList className="size-4" aria-hidden />} title={t('employer.campaigns.wizard.steps.criteria')} headerAside={<CampaignRubricTotalWeight totalWeight={totalWeight} totalMaxScore={totalMaxScore} weightValid={weightValid} maxScoreValid={maxScoreValid} hasCriteria={rubric.length > 0} resetDisabled={Boolean(isSaving)} onReset={onReset} />} footer={<CampaignWizardNav onBack={onBack} onNext={onNext} isSaving={isSaving} nextDisabled={!canNext} />}>
    <div className="space-y-6">
      ...
      {shouldShowRubricSummary(rubric) ? <div className="rounded-lg border border-satin bg-surface-overlay px-3 py-2 text-xs text-muted-foreground">{t('employer.campaigns.wizard.rubric.summary').replace('{{anchors}}', String(rubric.filter((item) => item.levels?.length).length)).replace('{{total}}', String(rubric.length)).replace('{{floors}}', String(rubric.filter((item) => item.minPct != null).length))}</div> : null}
      <div className="grid gap-3 md:grid-cols-2"><SelectionOption title={t('employer.campaigns.wizard.criteriaSource.manual')} description={t('employer.campaigns.wizard.criteriaSource.manualDesc')} selected={source === 'manual'} onClick={() => setSource('manual')} showChevron={false} /><SelectionOption title={t('employer.campaigns.wizard.criteriaSource.system')} description={t('employer.campaigns.wizard.criteriaSource.systemDesc')} selected={source === 'system'} onClick={() => { setSource('system'); setPreviewOpen(true); }} showChevron={false} disabled={criteriaQuery.isFetching || !jobCategory} className={rubric.length === 0 ? 'min-h-[128px] border-2 border-satin bg-surface-raised shadow-[var(--shadow-lg)]' : undefined} icon={criteriaQuery.isFetching ? <Loader2 className="size-5 animate-spin" /> : <Sparkles className="size-5" />} /></div>
      <CampaignCriteriaManualList rubric={rubric} disabled={Boolean(isSaving)} onChangeRubric={onChangeRubric} />
    </div>
