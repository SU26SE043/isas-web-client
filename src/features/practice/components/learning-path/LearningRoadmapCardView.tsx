...
export const LearningRoadmapCardView: React.FC<{ roadmap: LearningRoadmapCard }> = ({ roadmap }) => {
  ...
  return <article className="group relative flex flex-col overflow-hidden rounded-2xl border border-satin bg-surface-raised p-5 shadow-none transition-[border-color,box-shadow,transform] duration-200 hover:-translate-y-0.5 hover:border-info/45 hover:shadow-none sm:p-7">
    <div className="flex items-start justify-between gap-4"><div className="flex min-w-0 items-center gap-4"><div className="flex size-12 shrink-0 items-center justify-center rounded-full border border-info/50 bg-info/10 text-info shadow-[0_0_24px_-10px_var(--color-info)]"><GitBranch className="size-6" aria-hidden /></div><div className="min-w-0"><h2 className="line-clamp-2 break-words text-xl font-semibold text-foreground" title={name}>{name}</h2><p className="mt-1 truncate text-sm text-muted-foreground" title={`${domain} · ${t(`practice.roadmapWizard.level.${roadmap.targetLevel}`)}`}>{domain} · {t(`practice.roadmapWizard.level.${roadmap.targetLevel}`)}</p></div></div><span className={`inline-flex shrink-0 items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium ${statusClass}`}><span className="size-1.5 rounded-full bg-current" aria-hidden />{t(`practice.learningPath.status.${roadmap.status}`)}</span></div>
