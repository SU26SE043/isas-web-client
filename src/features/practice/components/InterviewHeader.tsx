...
export const InterviewHeader: React.FC<InterviewHeaderProps> = ({ sessionId }) => {
  const { t } = useLanguage();
    ...
    <header className="bg-surface-raised px-6 py-3 flex items-center justify-between sticky top-0 z-50 shadow-md">
      ...
      <div className="flex items-center gap-5">
        <div className="flex items-center gap-2 bg-surface-overlay/40 px-4 py-1.5 rounded-lg border border-default ">
          <svg className="w-4 h-4 text-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} aria-hidden>
        ...
        <button
          type="button"
          className="px-4 py-2 bg-surface-overlay text-foreground rounded-lg text-sm font-black hover:bg-surface-elevated transition-colors shadow-sm cursor-pointer"
