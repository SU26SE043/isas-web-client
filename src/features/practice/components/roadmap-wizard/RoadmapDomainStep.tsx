...
export const RoadmapDomainStep: React.FC<RoadmapDomainStepProps> = ({
  ...
  return (
    <section className="rounded-xl border border-subtle bg-surface-raised p-6">
      <h2 className="heading-secondary text-lg">{t('practice.roadmapWizard.domain.title')}</h2>
      <p className="body-text mt-1 text-sm">{t('practice.roadmapWizard.domain.description')}</p>
      ...
      <RoadmapWizardNav nextDisabled={!selectedId} onNext={onNext} backDisabled />
    </section>
  );
};
