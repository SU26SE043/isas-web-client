...
export function RoadmapWizardPage() {
  ...
  return (
    <RoadmapWizardShell currentStep={Math.max(flow.steps.indexOf(flow.step), 0)} stepKeys={stepKeys}>
      ...
      {flow.step === 'reports' ? <RoadmapReportsStep reports={flow.allReports} reportCounts={flow.reportCounts} selectedDomainId={flow.domainId} selectedIds={flow.selectedIds} isLoading={flow.loadingReports} loadError={flow.reportsError} onToggle={flow.toggleReport} onSelectAll={flow.selectAllReports} onUnselectAll={flow.unselectAllReports} onBack={flow.goBack} onNext={flow.goNext} goToStep={flow.goToStep} /> : null}
